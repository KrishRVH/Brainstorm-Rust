...
const std::array<Pack, 15> kPackInfo = {
...
void Instance::lock(Item item) {
    locked[static_cast<std::size_t>(item)] = true;
}
void Instance::unlock(Item item) {
    locked[static_cast<std::size_t>(item)] = false;
}
bool Instance::isLocked(Item item) const {
    return locked[static_cast<std::size_t>(item)];
}
...
// Card Generators
Item Instance::nextTarot(const std::string& source, int ante, bool soulable) {
...
Item Instance::nextPlanet(const std::string& source, int ante, bool soulable) {
    std::string anteStr = anteToString(ante);
...
Item Instance::nextSpectral(const std::string& source, int ante, bool soulable) {
    std::string anteStr = anteToString(ante);
    if (soulable) {
...
JokerData Instance::nextJoker(const std::string& source, int ante, bool hasStickers) {
    std::string anteStr = anteToString(ante);
...
// Shop Logic
ShopInstance Instance::getShopInstance() const {
    double tarotRate = 4;
    double planetRate = 4;
...
static Item shopItemType(const ShopInstance& shop, double cdtPoll) {
...
Pack packInfo(Item pack) {
    return kPackInfo[static_cast<std::size_t>(pack) - static_cast<std::size_t>(Item::Arcana_Pack)];
}
...
// Misc
bool Instance::isVoucherActive(Item voucher) const {
    return params
        .vouchers[static_cast<std::size_t>(voucher) - static_cast<std::size_t>(Item::Overstock)];
}

void Instance::activateVoucher(Item voucher) {
    params.vouchers[static_cast<std::size_t>(voucher) - static_cast<std::size_t>(Item::Overstock)] =
        true;
    lock(voucher);
    // Unlock next level voucher
    for (std::size_t i = 0; i < VOUCHERS.size(); i += 2) {
