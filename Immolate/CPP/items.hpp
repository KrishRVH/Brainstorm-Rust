...
inline Item stringToItem(const std::string& i) {
    if (i == "RETRY") {
        return Item::RETRY;
    };
    if (i == "J BEGIN") {
...
struct ShopInstance {
    ...
    ShopInstance() = default;
    ShopInstance(double j, double t, double p, double c, double s)
        : jokerRate(j), tarotRate(t), planetRate(p), playingCardRate(c), spectralRate(s) {}
...
struct JokerStickers {
    ...
    bool rental{false};
    JokerStickers() = default;
    JokerStickers(bool e, bool p, bool r) : eternal(e), perishable(p), rental(r) {}
};

struct JokerData {
    Item joker{Item::Joker};
    Item rarity{Item::Common};
    Item edition{Item::No_Edition};
    JokerStickers stickers{};
    JokerData() = default;
    JokerData(Item j, Item r, Item e, JokerStickers s)
        : joker(j), rarity(r), edition(e), stickers(s) {}
};

struct ShopItem {
    Item type{Item::T_Tarot};
    Item item{Item::The_Fool};
    JokerData jokerData{};
    ShopItem() = default;
    ShopItem(Item t, Item i) : type(t), item(i) {}
    ShopItem(Item t, Item i, JokerData j) : type(t), item(i), jokerData(j) {}
};

struct WeightedItem {
    Item item{Item::RETRY};
    double weight{0.0};
    constexpr WeightedItem(Item i, double w) : item(i), weight(w) {}
};

struct Pack {
    Item type{Item::Arcana_Pack};
    int size{0};
    int choices{0};
    Pack() = default;
    Pack(Item t, int s, int c) : type(t), size(s), choices(c) {}
};

struct Card {
    Item base{Item::C_2};
    Item enhancement{Item::No_Enhancement};
    Item edition{Item::No_Edition};
    Item seal{Item::No_Seal};
    Card(Item b, Item n, Item e, Item s) : base(b), enhancement(n), edition(e), seal(s) {}
};
...
inline constexpr std::array<WeightedItem, 16> PACKS = {
